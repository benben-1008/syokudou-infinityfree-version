...
switch ($_SERVER['REQUEST_METHOD']) {
    ...
    case 'DELETE':
        ...
        $shifts = read_json($file);
        $shifts = array_filter($shifts, function($shift) use ($data) {
            return ($shift['id'] ?? null) !== $data['id'];
        });
        
        write_json($file, array_values($shifts));
        echo json_encode(['ok' => true], JSON_UNESCAPED_UNICODE);
        break;
